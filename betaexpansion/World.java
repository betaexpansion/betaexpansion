    ...
    implements IBlockAccess
{
    ...
    public boolean markBlockDirty(int i, int j, int k){
        if(i < 0xfe17b800 || k < 0xfe17b800 || i >= 0x1e84800 || k > 0x1e84800)
        {
            return false;
        }
    ...
    {
        if (!worldInfo.getRaining() && !worldInfo.getThundering()){
            return;
        }
    ...
    protected void updateBlocksAndPlayCaveSounds()
    {
        ...
        for(Iterator iterator = positionsToUpdate.iterator(); iterator.hasNext();)
        {
            ...
            if(rand.nextInt(0x186a0) == 0 && func_27161_C() && func_27160_B())
            {
                ...
                int range = Config.LIGHT_RANGE;
                for (int xOff = -range; xOff <= range; xOff++){
                    for (int zOff = -range; zOff <= range; zOff++){
                        i5 = Math.max(i5, findTopSolidBlock(i3 + xOff,
                                                            i4 + zOff));
                    }
                }
                range *= 2;
                ...
                for (int xOff = -range; xOff <= range; xOff++){
                    for (int zOff = -range; zOff <= range; zOff++){
                        tmpY = findTopSolidBlock(i3 + xOff, i4 + zOff);
                        if (getBlockMaterial(i3 + xOff, tmpY, i4 + zOff) == Material.iron){
                            i5 = Math.max(i5, tmpY);
                        }
                    }
                }
            ...
            if(rand.nextInt(16) == 0)
            {
                ...
                if(getWorldChunkManager().getBiomeGenAt(j3 + k, j4 + i1).getEnableSnow() && j5 >= 0 && j5 < 128 && chunk.getSavedLightValue(EnumSkyBlock.Block, j3, j5, j4) < 10)
                {
                    ...
                    if (k6 == Block.snow.blockID && SeasonManager.winter){
                    	if (getBlockMetadata(j3 + k, j5 - 1, j4 + i1) < 16){
                    		setBlockMetadataWithNotify(j3 + k, j5, j4 + i1, getBlockMetadata(j3 + k, j5, j4 + i1) + 1);
                    	}
                    }
                    
                    if (Block.blocksList[getBlockId(j3 + k, j5, j4 + i1)] instanceof BlockFlower){
                    	setBlockWithNotify(j3 + k, j5, j4 + i1, Block.snow.blockID);
                    }
                }
            }
