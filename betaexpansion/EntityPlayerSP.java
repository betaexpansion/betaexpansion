...
public class EntityPlayerSP extends EntityPlayer
{
    ...
    protected boolean pushOutOfBlocks(double d, double d1, double d2)
    {
        ...
        {
            if (!(worldObj.getBlockId(i, j, k) == BEBlocks.quickSand.blockID
               && worldObj.getBlockId(i, j + 1, k) ==
               BEBlocks.quickSand.blockID)){
                return false;
               } // I don't know why this works, if something
                 // is broken with collisions it's probably something
                 // about this!
            ...
            int qs = BEBlocks.quickSand.blockID;

            flag  &= worldObj.getBlockId(i - 1, j, k) != qs &&
                    worldObj.getBlockId(i - 1, j + 1, k) != qs;
            
            flag1 &= worldObj.getBlockId(i + 1, j, k) != qs &&
                    worldObj.getBlockId(i + 1, j + 1, k) != qs;
            
            flag2 &= worldObj.getBlockId(i, j, k - 1) != qs &&
                    worldObj.getBlockId(i, j + 1, k - 1) != qs;
            
            flag3 &= worldObj.getBlockId(i, j, k + 1) != qs &&
                    worldObj.getBlockId(i, j + 1, k + 1) != qs;
