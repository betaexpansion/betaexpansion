...
public class PlayerControllerSP extends PlayerController
{
    ...
    public boolean sendBlockRemoved(int i, int j, int k, int l)
    {
        ...
        if(itemstack != null)
        {
        	 if (mc.thePlayer.getCurrentEquippedItem().getItem().shiftedIndex == BEItems.chisel.shiftedIndex
        			 && itemstack.getItem().canHarvestBlock(Block.blocksList[i1])){
        		 flag1 = false;
        	 }
            itemstack.onDestroyBlock(i1, i, j, k, mc.thePlayer);
            itemstack.getItem().breakBlockWith(i, j, k, i1, j1, mc.theWorld, mc.thePlayer);
