...
public class BlockFire extends Block
{
    ...
    ///////////////////////////////////////////////////
    // beta expansion blocks are loaded after vanilla
    // ones, so we need a seperate method for them
    @Override
    protected void BEInit(){
        setBurnRate(BEBlocks.wildGrass.blockID, 60, 100);
        setBurnRate(BEBlocks.roots.blockID, 60, 100);
        setBurnRate(BEBlocks.berryBush.blockID, 60, 100);
        setBurnRate(BEBlocks.peanutCrop.blockID, 60, 100);
        setBurnRate(BEBlocks.cornCrop.blockID, 60, 100);
        setBurnRate(BEBlocks.radishCrop.blockID, 60, 100);
        setBurnRate(BEBlocks.weakThatch.blockID, 60, 100);
        setBurnRate(BEBlocks.thatch.blockID, 60, 100);
    }
    ...
    public void updateTick(World world, int i, int j, int k, Random random)
    {
        ...
        if(!flag && random.nextInt(Config.FIRE_RAIN_LIFE) == 0 && world.func_27161_C() && (world.canBlockBeRainedOn(i, j, k) || world.canBlockBeRainedOn(i - 1, j, k) || world.canBlockBeRainedOn(i + 1, j, k) || world.canBlockBeRainedOn(i, j, k - 1) || world.canBlockBeRainedOn(i, j, k + 1)))
        {
            world.setBlockWithNotify(i, j, k, 0);
            return;
        }
